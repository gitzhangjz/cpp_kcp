...
static inline I64 iclock64(void)
{
	long s, u;
	I64 value;
	itimeofday(&s, &u);
	value = ((I64)s) * 1000 + (u / 1000);
	return value;
}

static inline U32 iclock()
{
	return (U32)(iclock64() & 0xfffffffful);
}
...
public:
	...
	U32 ts() const { return _ts; }
	void setts(U32 ts) { _ts = ts; }
...
public:
	...
	void send(int peer, const void *data, int size) {
		...
		if (peer == 0) {
			p12.push_back(pkt);
            // printf("peer %d send: %d, p12size:%d\n\n", peer, size, p12.size());
		}	else {
			p21.push_back(pkt);
		}
	}
